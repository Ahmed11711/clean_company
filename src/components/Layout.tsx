...
const INITIAL_NOTIFICATIONS: Notification[] = [
  {
    id: 1,
    title: 'New Booking',
    message: 'Ahmed Samir booked a Deep Cleaning service for tomorrow at 10:00 AM.',
    time: '2 mins ago',
    isRead: false,
    type: 'info',
  },
  ...
  },
  {
    id: 3,
    title: 'Staff Alert',
    message: 'Staff member John Doe is running late for his 2:00 PM appointment.',
    time: '3 hours ago',
    isRead: true,
    type: 'warning',
  },
          ...
          <div className="flex h-16 items-center border-b border-border-light px-6 justify-between">
            <div className="flex items-center gap-2">
              <div className="h-8 w-8 rounded-lg bg-emerald-solid flex items-center justify-center">
                <span className="text-white font-bold text-xs tracking-tighter" style={{ fontFamily: "'Brush Script MT', cursive" }}>C</span>
              </div>
              <span className="text-lg font-bold tracking-tight text-carbon-black">Cleany</span>
            </div>
            <Button variant="ghost" size="sm" onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-1">
          ...
          <nav className="flex-1 space-y-1 px-3 py-6 overflow-y-auto">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <item.icon className={cn("h-5 w-5", isActive ? "text-emerald-solid" : "text-slate-400")} />
                  {item.label}
                </Link>
              );
            })}
      ...
      <div className="flex flex-1 flex-col overflow-hidden relative">
        ...
        <header className="flex h-16 items-center justify-between border-b border-border-light bg-white px-4 lg:px-6 shrink-0">
          <div className="flex items-center gap-3 lg:hidden">
            ...
            <span className="text-base font-bold tracking-tight text-carbon-black">Cleany</span>
          </div>
          ...
          <div className="flex flex-1 items-center justify-end gap-2 lg:gap-4">
            <div className="relative">
              <Button 
                ...
                className={cn(
                  ...
                  isNotificationsOpen ? "bg-emerald-tint text-emerald-text" : "hover:bg-slate-50"
                )}
              >
                <Bell className={cn("h-5 w-5", isNotificationsOpen ? "text-emerald-solid" : "text-slate-400")} />
                {unreadCount > 0 && (
            ...
            <div className="flex items-center gap-2 lg:gap-3 pl-2">
              <div className="text-right hidden sm:block">
                <p className="text-sm font-semibold text-carbon-black leading-none">Ahmed Samir</p>
                <p className="text-[10px] text-slate-400 mt-1 capitalize">{role}</p>
              </div>
        ...
        <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-border-light px-2 py-1 z-50 flex items-center justify-around shadow-[0_-4px_12px_rgba(0,0,0,0.03)]">
          ...
          >
            <Menu className={cn("h-5 w-5", isSidebarOpen ? "stroke-[2.5px]" : "stroke-[1.5px]")} />
